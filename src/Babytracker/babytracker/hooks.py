...
def detect_mobile(event):
    request = event.request

    userAgent = request.headers.get('User-Agent', None)
    cookie = request.cookies.get('preferred-view', None)

    if cookie == 'mobile':
        alsoProvides(request, IMobileRequest)
    elif cookie == 'desktop':
        alsoProvides(request, IDesktopRequest)
    elif userAgent is not None and (reg_b.search(userAgent) or reg_v.search(userAgent[0:4])):
        alsoProvides(request, IMobileRequest)
    else:
        alsoProvides(request, IDesktopRequest)
...
def add_base_template(event):
    if IMobileRequest.providedBy(event['request']):
        layout = get_renderer('templates/mobile_layout.pt').implementation()
